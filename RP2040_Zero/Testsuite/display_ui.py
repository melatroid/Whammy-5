...
def init(
    ...
    pg_items = []
    if callable(bluetooth_test):
        pg_items.append(MenuItem("BT Test", on_select=lambda u: (
            u._toast("BT", "test...", 300),
            run_with_timeout(bluetooth_test, 8000, "BT test"),
            u._toast("BT", "done", 300)
        )))
    if callable(sdcard_test):
        pg_items.append(MenuItem("SD Test", on_select=lambda u: (
            u._toast("SD", "test...", 300),
            run_with_timeout(sdcard_test, 8000, "SD test"),
            u._toast("SD", "done", 300)
        )))
    pg_items.append(MenuItem("OLED Clear", on_select=lambda u: (oled_clear(), u._toast("OLED", "cleared", 250))))
    pg_items.append(MenuItem("Back", on_select=lambda u: u.pop_menu()))
    playground_menu = Menu("Playground", pg_items)
